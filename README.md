...
→ Best fit: OpenAI Pro 20x at $200/mo — fits below baseline
...
- **Sessions line**: total distinct JSONL files (≈ Claude sessions) seen,
  ...
  Pro 20x rather than Claude Max 20x.
...
- **Tests**: there is no test suite yet — a unit harness around
  `normalizeModel`, `verdict5h`, `findBestFit`, and `computeRows` would
  pay off quickly.
